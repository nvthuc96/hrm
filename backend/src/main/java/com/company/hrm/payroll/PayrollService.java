...
public class PayrollService {
    ...
    private static final BigDecimal INSURANCE_RATE = new BigDecimal("0.105"); // BHXH+BHYT+BHTN (nhân viên)
    private static final BigDecimal PERSONAL_DEDUCTION = new BigDecimal("11000000"); // giảm trừ bản thân
    private static final BigDecimal TAX_RATE = new BigDecimal("0.10"); // thuế TNCN đơn giản hóa (flat)
    ...
    public PayrollService(PayrollPeriodRepository periodRepository,
    ...
    public PayrollPeriodResponse createPeriod(int month, int year) {
        periodRepository.findByMonthAndYear(month, year).ifPresent(p -> {
    ...
    public PayrollPeriodResponse setStatus(Long periodId, PeriodStatus status) {
        PayrollPeriod p = periodRepository.findById(periodId)
    ...
    public PayslipResponse getPayslip(Long id) {
        return PayrollMapper.toResponse(payslipRepository.findById(id)
    ...
    public byte[] exportXlsx(Long periodId) {
        ...
        List<List<Object>> rows = listPayslips(periodId).stream()
        ...
        List<String> headers = List.of(
                "Nhân viên", "Kỳ", "Ngày công", "Lương cơ bản", "Tổng phụ cấp",
                "Tổng khấu trừ", "Gross", "BHXH", "Thuế TNCN", "Thực nhận");
        return ExcelExporter.toXlsx("Bảng lương " + period.getMonth() + "-" + period.getYear(), headers, rows);
    }
    ...
    private Payslip buildPayslip(Employee employee, PayrollPeriod period, List<SalaryComponent> components) {
        // Replace any existing payslip so regeneration is idempotent.
        payslipRepository.findByEmployeeIdAndPeriodId(employee.getId(), period.getId())
                .ifPresent(payslipRepository::delete);
        payslipRepository.flush();

        BigDecimal baseSalary = contractRepository
        ...
        int workingDays = (int) attendanceRepository.countByEmployeeIdAndStatusAndWorkDateBetween(
                employee.getId(), AttendanceStatus.PRESENT, from, to);

        // Pro-rate base by attendance (capped at a full month).
        BigDecimal ratio = BigDecimal.valueOf(workingDays)
        ...
        BigDecimal taxableIncome = proratedBase.add(taxableAllowance)
                .subtract(insurance).subtract(PERSONAL_DEDUCTION);
        BigDecimal tax = taxableIncome.signum() > 0
                ? scale(taxableIncome.multiply(TAX_RATE))
                : BigDecimal.ZERO;
