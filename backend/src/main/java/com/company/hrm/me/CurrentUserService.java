 ...
 * Used by self-service endpoints so a logged-in user can only touch their own data.
 */
@Service
public class CurrentUserService {
    ...
    /**
     * The employee id linked to the current account. Employees self-service only
     * works when the account is linked to an employee record.
     */
    public Long requireEmployeeId() {
        Long employeeId = requireUser().getEmployeeId();
        if (employeeId == null) {
            throw new BusinessException("Tài khoản của bạn chưa được liên kết với hồ sơ nhân viên. "
