...
/**
 * Builds the refresh-token cookie. It is httpOnly (invisible to JS, so XSS can't read it), scoped
 * to the auth endpoints, and — via SameSite — not sent on cross-site requests, which neutralises
 * CSRF against refresh/logout.
 */
@Component
public class AuthCookies {
  ...
  public ResponseCookie build(String refreshToken) {
    return base(refreshToken).maxAge(Duration.ofSeconds(refreshMaxAgeSeconds)).build();
  }
