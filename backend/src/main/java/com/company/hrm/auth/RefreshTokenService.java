...
/**
 * Issues, validates, rotates and revokes opaque refresh tokens. The raw token
 * is returned to the caller once (on issue/rotate); only its SHA-256 hash is
 * persisted, and validation always checks expiry + revocation.
 */
@Service
@Transactional
public class RefreshTokenService {
    ...
    /**
     * Rotate: validate the presented token, revoke it, and issue a fresh one for
     * the same user. Returns the new raw token. Rotation limits the window in
     * which a stolen refresh token is usable.
     */
