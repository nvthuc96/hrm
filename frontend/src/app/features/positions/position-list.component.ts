      ...
      <div class="tbl-scroll"><table mat-table [dataSource]="data()" class="w-full">
        ...
        <ng-container matColumnDef="actions">
          ...
          <td mat-cell *matCellDef="let p" class="text-right">
            <button mat-icon-button [matTooltip]="'common.edit' | translate" (click)="openForm(p)">
              <mat-icon>edit</mat-icon>
            </button>
            <button mat-icon-button color="warn" [matTooltip]="'common.delete' | translate" (click)="confirmDelete(p)">
...
export class PositionListComponent implements OnInit {
  ...
  openForm(position?: Position): void {
    ...
    ref.afterClosed().subscribe((saved) => {
      if (saved) {
        this.snackBar.open(this.i18n.t(position ? 'pos.updated' : 'pos.added'), this.i18n.t('common.ok'), {
          duration: 2500,
        });
        this.load();
      }
    ...
    ref.afterClosed().subscribe((ok) => {
      if (ok) {
        this.service.delete(position.id).subscribe({
          next: () => {
            this.snackBar.open(this.i18n.t('pos.deleted'), this.i18n.t('common.ok'), { duration: 2500 });
            this.load();
          },
          error: (err) =>
