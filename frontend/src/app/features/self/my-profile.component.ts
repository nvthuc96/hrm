    ...
    @if (profile()?.employee; as emp) {
      <div class="surface p-5 mb-5">
        <div class="flex items-center gap-4 mb-5">
          <span
            class="grid place-items-center w-14 h-14 rounded-2xl text-white text-lg font-semibold shrink-0"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
          <div>
            <div class="text-xs text-[var(--muted)]">{{ 'me.profile.dept' | translate }}</div>
            <div class="text-[var(--ink-soft)]">{{ emp.departmentName || '—' }}</div>
          </div>
          <div>
            <div class="text-xs text-[var(--muted)]">{{ 'me.profile.position' | translate }}</div>
            <div class="text-[var(--ink-soft)]">{{ emp.positionName || '—' }}</div>
          </div>
          <div>
            <div class="text-xs text-[var(--muted)]">{{ 'me.profile.hireDate' | translate }}</div>
            <div class="text-[var(--ink-soft)]">{{ emp.hireDate || '—' }}</div>
          </div>
          <div>
            <div class="text-xs text-[var(--muted)]">{{ 'me.profile.email' | translate }}</div>
            <div class="text-[var(--ink-soft)] break-all">{{ emp.email || '—' }}</div>
          </div>
          <div>
            <div class="text-xs text-[var(--muted)]">{{ 'me.profile.phone' | translate }}</div>
            <div class="text-[var(--ink-soft)]">{{ emp.phone || '—' }}</div>
          </div>
          <div>
            <div class="text-xs text-[var(--muted)]">{{ 'me.profile.dob' | translate }}</div>
            <div class="text-[var(--ink-soft)]">{{ emp.dob || '—' }}</div>
          </div>
          <div>
            <div class="text-xs text-[var(--muted)]">{{ 'me.profile.gender' | translate }}</div>
            <div class="text-[var(--ink-soft)]">{{ genderLabel(emp.gender) }}</div>
          </div>
          <div class="sm:col-span-2">
            <div class="text-xs text-[var(--muted)]">{{ 'me.profile.address' | translate }}</div>
            <div class="text-[var(--ink-soft)]">{{ emp.address || '—' }}</div>
          </div>
        </div>
      </div>
...
export class MyProfileComponent implements OnInit {
  ...
  rolesText = computed(() =>
    (this.profile()?.roles ?? [])
      .map((r) => this.i18n.t('role.' + r.replace(/^ROLE_/, '')))
      .join(', '),
  );
